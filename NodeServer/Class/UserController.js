...
function UserController(){
	...
	this.index = function(request, res){
		this.connection.query('SELECT * FROM users', (err, result) => {
			res.json({status:'success','result':result});
		});
	}
	this.store = function(request, res){
		let body=[];
		let errors =[]
		request.on('data', (chunk) => {
			...
			if(body.email==undefined){
				...
				this.connection.query('SELECT * FROM users WHERE email=?', body.email, (err, emailResult) => {
					...
					if(errors.length==0){
						let newUser = {email:body.email,firstname:body.firstname,lastname:body.lastname,password:body.password};
	...
	this.show = function(request, res){
	...
	this.update = function(request, res){
		let body=[];
		request.on('data', (chunk) => {
			body.push(chunk);
		}).on('end', () => {
	...
	this.destroy = function(request, res){
